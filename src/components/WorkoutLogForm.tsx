...
const WorkoutLogForm: React.FC<WorkoutLogFormProps> = ({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 빈 운동명이나 세트 체크
    const hasEmptyExercises = exercises.some(ex => 
      !ex.name.trim() || ex.sets.some(set => !set.reps || !set.weight_kg)
    );
    ...
    try {
      ...
      for (const exercise of exercises) {
        const workoutData = exercise.sets.map(set => ({
  ...
  return (
    <div className={`${isEditMode ? '' : 'max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-md'}`}>
      ...
      {message && (
        <div className={`p-3 mb-6 rounded-lg border ${
          message.includes('성공') 
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <button
          type="button"
          onClick={addExercise}
          className="w-full flex items-center justify-center space-x-2 py-3 border-2 border-dashed border-gray-300 rounded-lg text-gray-600 hover:border-green-400 hover:text-green-600 transition-colors"
        >
